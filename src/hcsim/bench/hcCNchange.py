...
def calculate_CN_Change_accuracy(ground_truth,predict_data):
    ...
    return np.mean((parent-child) == (parent_predict-child_predict))

def calculate_CN_change_rmse(ground_truth,predict_data):
    parent = ground_truth.str.split('->').str[0].apply(pd.to_numeric, errors='coerce')
    parent_predict = predict_data.str.split('->').str[0].apply(pd.to_numeric, errors='coerce')
    ...
    rmse = np.sqrt(mean_squared_error(parent-child, parent_predict-child_predict))
...
def run(args):
    ...
    for tool_maternal_path,tool_paternal_path, tool_name in zip(args.tool_maternal_cna_files, args.tool_paternal_cna_files,args.tool_names):
        ...
        for type in combined_result['Type'].unique().tolist():
            ...
            rmse = calculate_CN_change_rmse(combined_result[combined_result['Type']==type]['Change'], combined_result[combined_result['Type']==type]['Change_predict'])
            acc = calculate_CN_Change_accuracy(combined_result[combined_result['Type']==type]['Change'], combined_result[combined_result['Type']==type]['Change_predict'])
    ...
    output_file = os.path.join(args.output_dir, "evolution_onset_CN_Change.csv")
