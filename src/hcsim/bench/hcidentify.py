# commands/hcidentify.py
import numpy as np
import pandas as pd
import os


def add_hcidentify_subparser(subparsers):
    """
    Add the hcidentify subcommand to the CLI tool.

    This subcommand is used to identify whether CNA events are correctly detected at their first branch of evolution, using ACC.
    """
    parser = subparsers.add_parser(
        "hcidentify",
        help="Identify whether CNA events are correctly detected at their first branch of evolution, using ACC."
    )
    parser.add_argument(
...
def calculate_Evolution_Onset_accuracy(ground_truth,predict_data):
    return np.mean(predict_data == ground_truth)


##python src/main.py hcidentify 
...
def run(args):
    ...
    for tool_maternal_path,tool_paternal_path, tool_name in zip(args.tool_maternal_cna_files, args.tool_paternal_cna_files,args.tool_names):
        ...
        for type in combined_result['Type'].unique().tolist():
            ...
            acc = calculate_Evolution_Onset_accuracy(combined_result[combined_result['Type']==type]['Change'], combined_result[combined_result['Type']==type]['Change_predict'])
    ...
    output_file = os.path.join(args.output_dir, "evolution_onset_acc.csv")
