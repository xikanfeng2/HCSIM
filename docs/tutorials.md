...
```{note}
The complete HCSIM pipeline will sequentially execute the following modules in order: gprofile -> gfasta -> gfastq -> align -> downsam -> pbam -> bcbam|rdr|baf. If you want to re-run one of these steps, please make sure the previous commands have been successfully executed.
```
...
| [bcbam](man/hcsim-bcbam.md) | Generating barcode BAM file | One or more running directories of previous runs of `pbam` |
| [rdr](man/hcsim-rdr.md) | Computing RDRs | One or more running directories of previous runs of `pbam` |
| [baf](man/hcsim-baf.md) | Computing BAFs | One or more running directories of previous runs of `pbam` |
...
# equivalent to the hcsim rdr subcommand
hcsim.rdr()

# equivalent to the hcsim baf subcommand
hcsim.baf()
```
...
7. `barcode_bam_log.txt`: the log information generated by `hcsim bcbam` command.
